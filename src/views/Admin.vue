...
<template>
  <div class="mx-auto max-w-7xl px-6 py-16">
    <header class="mb-10 text-center">
      <p class="mb-3 text-sm font-medium uppercase tracking-[0.2em] text-terracotta">Admin</p>
    ...
    <div class="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      <div class="space-y-6 rounded-[28px] border border-ink/5 bg-white p-6 shadow-[0_20px_50px_rgba(33,26,22,0.05)]">
        <div>
          ...
          <input v-model="draft.title" @input="updateSlug" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" placeholder="Titolo dell'articolo" />
        </div>

        <div>
          <label class="mb-2 block text-sm font-medium text-ink">Slug</label>
          <input v-model="draft.slug" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" placeholder="slug-articolo" />
        </div>

        <div>
          <label class="mb-2 block text-sm font-medium text-ink">Abstract / Excerpt</label>
          <textarea v-model="draft.excerpt" rows="3" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" placeholder="Breve intro del post" />
        </div>

        <div class="grid gap-4 sm:grid-cols-2">
          <div>
            <label class="mb-2 block text-sm font-medium text-ink">Regione</label>
            <input v-model="draft.region" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" placeholder="Toscana" />
          </div>
          <div>
            <label class="mb-2 block text-sm font-medium text-ink">Anno</label>
            <input v-model="draft.publishedAt" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" placeholder="2026" />
          </div>
          <div>
            <label class="mb-2 block text-sm font-medium text-ink">Tempo lettura</label>
            <input v-model="draft.readTime" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" placeholder="4 min lettura" />
          </div>
          <div>
            <label class="mb-2 block text-sm font-medium text-ink">Categoria</label>
            <input v-model="draft.category" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" placeholder="Viaggio" />
          </div>
        </div>

        <div>
          <label class="mb-2 block text-sm font-medium text-ink">URL immagine</label>
          <input v-model="draft.image" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" placeholder="/images/blog-import/xxx.jpg o https://..." />
        </div>

        <div>
          <div class="mb-3 flex items-center justify-between">
            <label class="block text-sm font-medium text-ink">Contenuto</label>
            <button type="button" @click="addParagraph" class="rounded-full border border-terracotta/30 bg-terracotta/5 px-3 py-1.5 text-xs font-medium text-terracotta">+ paragrafo</button>
          </div>

          <div v-for="(_, index) in draft.body" :key="index" class="mb-3 flex gap-3">
            <textarea v-model="draft.body[index]" rows="4" class="w-full rounded-2xl border border-ink/10 bg-cream px-4 py-3 outline-none focus:border-terracotta" :placeholder="`Paragrafo ${index + 1}`" />
        ...
        <div class="flex flex-wrap gap-3">
          <button type="button" @click="exportDraft" class="rounded-full bg-terracotta px-5 py-3 text-sm font-medium text-white hover:bg-terracotta-dark">Salva in locale</button>
          <router-link to="/blog" class="rounded-full border border-ink/10 bg-white px-5 py-3 text-sm font-medium text-ink hover:text-terracotta">Vedi blog</router-link>
        </div>
      </div>
